let cauliflower; // Make cauliflower accessible globally
...
const loadCauliflower = function(loader, textureLoader, objMaterial) {
    return new Promise((resolve, reject) => {
        loader.load('/assets/models/cauliflower_head.obj', function(object) {
            const colorMap = textureLoader.load('/assets/models/cauliflower_head.jpg');
...
async function init() {
    ...
    cauliflower = await loadCauliflower(loader, textureLoader, objMaterial);
    cauliflower.visible = false; // Start hidden
    scene.add(cauliflower);
...
function update(renderer, scene, camera, clock) {
    ...
    if (sceneCameraGroup) {
     ...
     // This prevents the update loop from overriding the click event
    if (!cauliflower || !cauliflower.visible) {
        const geoTypes = GEO_TYPES;
        ...
        // If cauliflower is visible, ensure all GEO_TYPES are hidden
        GEO_TYPES.forEach(function(geo) {
            const currentObj = scene.getObjectByName(geo);
            if (currentObj && currentObj.visible) {
